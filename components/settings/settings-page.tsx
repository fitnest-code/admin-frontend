...
const TABS = [
  ...
  { key: 'reservations',  label: 'Rezervasiya qaydaları', icon: CalendarCheck },
] as const
...
function ReservationsTab() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formMode === 'edit' && editCode) {
        ...
        setModalConfig({ isOpen: true, message: 'Ləğv səbəbi uğurla yeniləndi.', type: 'success' })
      } else {
        await createReason(payload)
        setModalConfig({ isOpen: true, message: 'Yeni ləğv səbəbi əlavə edildi.', type: 'success' })
      }
      setShowFormModal(false)
    } catch (err: any) {
      console.error(err)
      const msg = err?.response?.data?.error?.message || err?.message || 'Əməliyyat baş tutmadı. Zəhmət olmasa yenidən cəhd edin.'
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setModalConfig({ isOpen: true, message: 'Ləğv səbəbi uğurla silindi.', type: 'success' })
      setDeleteTargetCode(null)
    } catch (err: any) {
      console.error(err)
      const msg = err?.response?.data?.error?.message || err?.message || 'Silinmə zamanı xəta baş verdi.'
      setModalConfig({ isOpen: true, message: msg, type: 'error' })
    } finally {
      setIsActionLoading(false)
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      <Section title="Rezervasiya ləğv səbəbləri">
        <div className="flex justify-between items-center pb-2 gap-4">
          <p className="text-xs text-muted-foreground leading-relaxed">
            İstifadəçilər və adminlər tərəfindən rezervasiyanın ləğvi zamanı seçilən səbəblərin idarə olunması.
          </p>
          <button
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-sm text-muted-foreground italic border border-dashed border-border rounded-xl">
            Heç bir ləğv səbəbi təyin edilməyib
          </div>
        ) : (
          <div className="overflow-hidden rounded-xl border border-border">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-foreground">Səbəb kodu</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-foreground">Səbəb mətni (AZ)</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-foreground">Şərh tələb olunur</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-foreground w-28">Əməliyyat</th>
                </tr>
              </thead>
                  ...
                  <tr key={reason.code} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-center">
                      <span className={cn(
                        ...
                        {reason.requiresComment ? 'Bəli' : 'Xeyr'}
                      </span>
                    </td>
      ...
      {showFormModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <form
            onSubmit={handleSave}
            className="relative z-10 w-full max-w-[480px] rounded-2xl bg-white border border-[#ececed] p-6 flex flex-col gap-5 shadow-2xl"
          >
            <div className="flex items-center justify-between border-b border-[#ececed] pb-3 text-[#101828]">
              <h3 className="text-base font-semibold leading-tight">
                {formMode === 'create' ? 'Yeni ləğv səbəbi əlavə et' : 'Ləğv səbəbini redaktə et'}
              </h3>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-muted-foreground">Səbəb kodu</label>
                <input
                  type="text"
                  required
                  value={code}
                ...
                />
                <span className="text-[10px] text-muted-foreground font-medium">Böyük hərflərlə, boşluq yerinə _ işarəsi ilə yazın. Redaktə edilə bilməz.</span>
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-muted-foreground">Səbəb mətni (AZ)</label>
                <input
                  type="text"
                  required
                  value={label}
              ...
              <div className="flex items-center justify-between rounded-xl border border-border bg-[#fafafa] p-4">
                <div className="flex flex-col">
                  <span className="text-sm font-semibold text-foreground">Şərh tələb olunur</span>
                  <span className="text-[10px] text-muted-foreground leading-normal">İstifadəçi bu səbəbi seçdikdə əlavə rəy yazmalıdır.</span>
                </div>
            ...
            <div className="flex justify-end gap-3 border-t border-border pt-4 mt-2">
              ...
              >
                {isActionLoading ? <Loader2 size={16} className="animate-spin" /> : 'Yadda saxla'}
              </button>
            </div>
          </form>
        </div>
      ...
      {deleteTargetCode && (
        <ConfirmDeleteModal
          name={`"${deleteTargetCode}" kodu olan ləğv səbəbini`}
