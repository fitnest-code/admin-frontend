...
interface GirisQrTabProps {
  zalName: string
}

export function GirisQrTab({ zalName }: GirisQrTabProps) {
  // Static QR code URL using a free QR API
  const qrValue = encodeURIComponent(`fitnest-zal:${zalName}`)
  ...
  return (
    <div className="flex flex-col gap-5 py-4">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:gap-10">
          ...
          <div className="flex flex-col gap-3">
            <button className="flex items-center gap-3 rounded-lg border border-border bg-background px-4 py-3 text-sm font-medium text-foreground hover:border-[#00B4CC] hover:text-[#00B4CC] transition-colors group">
              <Download size={16} className="text-muted-foreground group-hover:text-[#00B4CC] transition-colors" aria-hidden />
              QR pdf yüklə
            </button>
            <button className="flex items-center gap-3 rounded-lg border border-border bg-background px-4 py-3 text-sm font-medium text-foreground hover:border-[#00B4CC] hover:text-[#00B4CC] transition-colors group">
              <RefreshCw size={16} className="text-muted-foreground group-hover:text-[#00B4CC] transition-colors" aria-hidden />
              QR yenilə
            </button>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between border-t border-border pt-5">
        ...
        <button
          disabled
          className="rounded-lg bg-secondary px-6 py-2.5 text-sm font-semibold text-muted-foreground cursor-not-allowed"
        >
          Yadda saxla
        </button>
      </div>
