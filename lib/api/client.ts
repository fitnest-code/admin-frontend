...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  const { body, headers, auth = true, params, _retried = false, ...init } = options
  const isAuthRoute = path.startsWith('/api/auth/')
  const requestUrl = `${makeUrl(path, isAuthRoute)}${makeQueryString(params)}`
  const usesLocalApiRoute = requestUrl.startsWith('/api/')
  ...
  const response = await fetch(requestUrl, {
    ...
    cache: 'no-store',
    credentials: usesLocalApiRoute ? 'include' : 'omit',
  })
